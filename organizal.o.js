...
(function($) {
	...
	$.fn.orgchart = function(options) {
		var defaultOptions = {
			'nodeRelationship': 'relationship',//节点之间的关系
			'nodeChildren': 'children', //子节点
			'depth': 999, //？
			'chartClass': '',
			'exportButton': false,
		...
		// append the export button  导出组织结构图
	...
	// detect the exist/display state of related node 检测关联节点的显示\隐藏
	...
	// show the parent node of the specified node 显示指定节点的祖先节点
	...
	// show the children nodes of the specified node 显示指定节点的子代节点
	...
	// hide the sibling nodes of the specified node  隐藏指定节点的兄弟节点
	...
	// show the sibling nodes of the specified node 显示指定节点的兄弟节点
	...
	function createNode(nodeData, level, opts) {
		var dtd = $.Deferred();
		// construct the content of node 构造节点内容
		...
		// append 4 directions arrows 四个方向的伸缩
		...
		// bind click event handler for the bottom edge 鼠标点击节点下拉框
		...
		// bind click event handler for the left and right edges 鼠标点击节点左右拉框
	...
	function buildHierarchy ($appendTo, nodeData, level, opts, callback) {
		...
		// Construct the inferior nodes and connectiong lines 建立次级节点和连接线
		if (hasChildren) {
	...
	// build the child nodes of specific node 建立指定节点的子节点
	...
	// build the parent node of specific node 建立指定节点的父节点
	...
	// build the sibling nodes of specific node 建立指定节点的兄弟节点
